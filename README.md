...
**`openRepoTools --install` is the single writer of everything it places** (lane-collision-protocol Amendment 9(b), adoption act 4b). It places twelve files into `${OPENREPOTOOLS_BIN_DIR:-~/.local/bin}` at 755 — `openRepoTools`, `park`, `resume`, `status`, `lane`, `lanes`, `lane-handoff`, `lanes-edit.sh`, `lane-start`, `lane-end`, `link-estates` and the shipped alias table `repos.tsv` — plus fourteen artifacts that are not in that directory: the `handoff`, `lane-swap` and `restart` skills and the `handoff`, `ctx` and `swap` command files, each at `${CLAUDE_PROFILES_HOME:-~/.claude-profiles}/shared/skills/<name>/SKILL.md` (or `.../shared/commands/<name>.md`), and a copy of each at `~/.claude/skills/<name>/SKILL.md` (or `~/.claude/commands/<name>.md`) for a bare `claude` outside the launcher, plus two merged entries in `~/.claude/settings.json` — the `SessionStart` entry and lane-collision-protocol Amendment 12's `UserPromptSubmit` name guard. **Those counts are not blindly trusted forever:** `scripts/setup-estate-commands.sh` keeps its own `TOOLS_FILES`, `SKILL_NAMES` and `COMMAND_NAMES` — hand-kept mirrors of the pinned shim's `INSTALLABLES`, `SKILLS` and `COMMANDS` arrays, since there is no `--list-installables` to generate them from — but checks each one against the shim's own array before installing anything (`check_shim_list`, Copilot's review of workBenches#91), so a shim that grew past what this script lists refuses the whole install rather than silently installing the new file with no preflight or verification. Nothing else in this repository writes any of those twenty-six: `scripts/setup-claude-profiles.sh` used to install the skill and ensure that hook entry, and adoption act 4b deleted both. The one `SessionStart` ensure that is NOT `--install`'s is the launcher's own, in each *profile's* `settings.json` — a different file for a different run. That merge refuses — placing nothing at all, exit 2, bin directory untouched — on one condition only: a `SessionStart` entry that itself runs `session-start` under a different command string, which is a second program running the same hook verb and would fire it twice (R-A9-8, narrowed from `lanes-edit.sh` to the verb by R-A9-14). An unrelated `SessionStart` entry is merged beside it and the install exits 0.
