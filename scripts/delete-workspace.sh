#!/bin/bash
# Intelligent workspace deleter for workBenches
# Automatically detects workspace type and routes to the appropriate deleter
...
check_for_project_scripts() {
    ...
    for ((i=0; i<5; i++)); do
        if [ -f "${current_dir}/scripts/delete-frappe-workspace.sh" ] && [ -d "${current_dir}/workspaces" ]; then
            # Found a Frappe project with its own workspace scripts
            echo "${current_dir}/scripts/delete-frappe-workspace.sh"
            return 0
        fi
...
echo ""
echo -e "${BLUE}=========================================="
echo "Workspace Deleter (Intelligent Router v${SCRIPT_VERSION})"
echo -e "==========================================${NC}"
echo ""

# Detect available workspace deleters
declare -A DELETER_SCRIPTS
declare -a AVAILABLE_TYPES

# Check Frappe
if [ -f "${WORKBENCHES_ROOT}/devBenches/frappeBench/scripts/delete-frappe-workspace.sh" ]; then
    AVAILABLE_TYPES+=("Frappe")
    DELETER_SCRIPTS["Frappe"]="${WORKBENCHES_ROOT}/devBenches/frappeBench/scripts/delete-frappe-workspace.sh"
fi

# Check Flutter (when available)
if [ -f "${WORKBENCHES_ROOT}/devBenches/flutterBench/scripts/delete-flutter-workspace.sh" ]; then
    AVAILABLE_TYPES+=("Flutter")
    DELETER_SCRIPTS["Flutter"]="${WORKBENCHES_ROOT}/devBenches/flutterBench/scripts/delete-flutter-workspace.sh"
fi

# Check .NET (when available)
if [ -f "${WORKBENCHES_ROOT}/devBenches/dotnetBench/scripts/delete-dotnet-workspace.sh" ]; then
    AVAILABLE_TYPES+=("DotNET")
    DELETER_SCRIPTS["DotNET"]="${WORKBENCHES_ROOT}/devBenches/dotnetBench/scripts/delete-dotnet-workspace.sh"
fi
...
if [ ${#AVAILABLE_TYPES[@]} -eq 0 ]; then
    die "No workspace deleters found in devBenches"
fi
...
if [ -z "$SELECTED_TYPE" ] && [ ${#AVAILABLE_TYPES[@]} -eq 1 ]; then
    ...
    log_info "Only ${SELECTED_TYPE} workspace deleter available, using it..."
# If explicit type specified as first argument
elif [ $# -gt 0 ] && [ -z "$SELECTED_TYPE" ]; then
    # Try to match argument to a workspace type
    for type in "${AVAILABLE_TYPES[@]}"; do
        if [[ "${1,,}" == "${type,,}" ]] || [[ "${1,,}" == "delete-${type,,}-workspace.sh" ]]; then
            SELECTED_TYPE="$type"
...
if [ -z "$SELECTED_TYPE" ]; then
    ...
    if [ ! "$AI_AVAILABLE" = true ] && command -v show_workspace_selection_tui >/dev/null 2>&1; then
        ...
        echo ""
        log_info "Which type of workspace do you want to delete?"
        echo ""
...
# Route to appropriate deleter
log_info "Deleting ${SELECTED_TYPE} workspace(s)..."
echo ""

DELETER_SCRIPT="${DELETER_SCRIPTS[$SELECTED_TYPE]}"

if [ ! -f "$DELETER_SCRIPT" ]; then
    die "Deleter script not found: $DELETER_SCRIPT"
fi

# Pass through all remaining arguments to the specific deleter
exec "$DELETER_SCRIPT" "$@"
