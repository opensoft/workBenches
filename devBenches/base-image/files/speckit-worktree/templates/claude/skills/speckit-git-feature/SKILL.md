...
Create a new git feature checkout for the given specification. In branch mode this creates or switches the current checkout to a feature branch. In worktree mode this creates a linked worktree rooted from the configured base branch. This command handles **checkout creation only** — the spec directory and files are created by the core `/speckit.specify` workflow.
...
Determine the checkout strategy from `.specify/extensions/git/git-config.yml`:

1. Read `checkout_mode`
2. If absent, default to `branch`
3. When `checkout_mode: worktree`, also read:
   - `base_branch`
   - `worktree_root`
